...
logical :: sexist
   ...
   real*4  statinum(nstat), para_esc(nstat),pott,kinn
    integer eof /0/,typ(1:natom),typ_check(1:natom),cyp(natom),icolor,k,Code,cur_rec ,iras,np(12),stat(nstat)
    character,allocatable:: struu(:)
    character*5 s1
    character*6 s2,c2
    character*200 sfile 
    character*150 sline
    integer*4 vecino_check(1:natom),num_vecino(1 :5), ncluster(1:natom),mcluster(0:nclu,2), & 
    lcluster(0:nclu), ncluster_o(1:natom), mcluster_o(0:nclu,2), pos_cluster,cl1,cl2    ! 1- только металл, 2 - есть аргон


    integer*4 ncluster_check(1:natom),mcluster_check(0:nclu,2),lcluster_check(0:nclu)

    integer*8 n1, n2, iarg
    integer*4 i,j,num_atom,nt, M, cur_num ,cur_clu, numj_all(1000), numt_t(1000),numj_o(1000),numjt(1000), &
    numt_o(1000),numt_all(1000)
    integer*4  nmax /10000/,natraso /5/,sostav(500)
  
    real*8 pot(1 :natom),kin(1: natom),kin_check(1: natom)
    integer cluster(1:nclu,1:nstat),cluster_o(1:nclu,1:nstat),clupro(1:nstat), &
    cluster_check(1:nclu,1:nstat)
...
!!!! my_comment   CALL lammps_open('',MPI_COMM_WORLD,lmp)
!!!!!!!!!my_comment   call lammps_file (lmp, 'in.1')
nt=1
...
! entro y esta pensando
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!Here we read the data on the duration of life of dimers from the previously recorded files
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         if (num_vecino(5).ne.num_atom ) then   !quitamos the clusteres of argon solo .and. num_vecino(5)<5000 
              ...
              if (mcluster(pos_cluster,1)<nstat) then
                ...
                !endif
                   cluster(i,mcluster(pos_cluster,1)) = num_atom
              endif
          endif
 ! else 
...
!!!!!!!!!! my comment  close(50000,status='delete')
...
!!!!!!!!my_comment !!!!  call lammps_command (lmp, 'run 50000 pre no post no')
